...
//
// Contract: contracts/openapi/plot-store.yaml x-tca-lifecycle
...
func awaitShutdown(server *http.Server) {
	...
	sig := <-quit
	log.Printf("[plot-store] Received %s — starting graceful shutdown", sig)
	...
	if err := server.Shutdown(ctx); err != nil {
		log.Printf("[plot-store] HTTP server shutdown error: %v", err)
	}

	log.Printf("[plot-store] Shutdown complete")
	os.Exit(0)
}
