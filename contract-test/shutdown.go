...
//
// Contract: contracts/openapi/contract-test.yaml x-tca-lifecycle
...
func awaitShutdown(server *http.Server) {
	...
	sig := <-quit
	log.Printf("[contract-test] Received %s — starting graceful shutdown", sig)
	...
	if err := server.Shutdown(ctx); err != nil {
		log.Printf("[contract-test] HTTP server shutdown error: %v", err)
	}

	// Step 3: Close Redis connection — all in-flight publishes have completed.
	if rdb != nil {
		if err := rdb.Close(); err != nil {
			log.Printf("[contract-test] Redis close error: %v", err)
		}
	}

	log.Printf("[contract-test] Shutdown complete")
	os.Exit(0)
}
