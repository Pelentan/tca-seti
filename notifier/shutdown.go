...
//
// Contract: contracts/openapi/notifier.yaml x-tca-lifecycle
...
func awaitShutdown(server *http.Server) {
	...
	sig := <-quit
	log.Printf("[notifier] Received %s — starting graceful shutdown", sig)
	...
	if err := server.Shutdown(ctx); err != nil {
		log.Printf("[notifier] HTTP server shutdown error: %v", err)
	}

	log.Printf("[notifier] Shutdown complete")
	os.Exit(0)
}
