...
//
// Contract: contracts/openapi/seti-observability.yaml x-tca-lifecycle
...
func awaitShutdown(server *http.Server) {
	...
	sig := <-quit
	log.Printf("[seti-observability] Received %s — starting graceful shutdown", sig)
	...
	if err := server.Shutdown(ctx); err != nil {
		log.Printf("[seti-observability] HTTP server shutdown error: %v", err)
	}

	// Step 3: Close Redis connection — all in-flight publishes have completed.
	if rdb != nil {
		if err := rdb.Close(); err != nil {
			log.Printf("[seti-observability] Redis close error: %v", err)
		}
	}

	log.Printf("[seti-observability] Shutdown complete")
	os.Exit(0)
}
