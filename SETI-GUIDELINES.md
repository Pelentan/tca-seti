...
K8s NetworkPolicy objects replace Docker Compose network isolation.  Two policies per namespace mirror the two-network Docker Compose model:

**`seti-internal` policy** — allows all pods in the `seti` namespace to communicate with each other.  All traffic is mTLS.

**`ac-net` policy** — allows the `augur-canis` pod to reach all pods in the namespace.  Services cannot reach each other through AC's network.

The gateway is the only service reachable from outside the namespace.  External access routes through Traefik ingress at `https://seti.tca.local`.  Traefik terminates browser-facing TLS using a cert generated by cert-forge for the `seti.tca.local` hostname.  The Traefik-to-gateway connection is standard TLS — the gateway presents its cert-forge instance cert, which includes `seti.tca.local` in its SANs.  Internal service-to-service traffic remains mTLS throughout.

No plain HTTP anywhere in the stack — not at the ingress boundary, not between Traefik and the gateway, not between internal services.
...
SETI runs on a shared k3d cluster named `tca` alongside other TCA constellations.  See `K8S-MIGRATION-PRIMER.md` for cluster setup and the full K8s deployment reference.

```bash
# One-time cluster setup (see K8S-MIGRATION-PRIMER.md)
...
helm install seti ./charts/seti -n seti -f charts/seti/values/dev.yaml --create-namespace
kubectl apply -f k3d-cluster/ingress/seti-ingress.yaml
```

Open `https://seti.tca.local`.  The browser will warn about the cert — the SETI CA is self-signed and not in the OS trust store.  This is expected in dev.

**Cert rotation in K8s:** cert-forge automatically rotates instance certs and the constellation CA every 30 days.  The rotation list in `forge.json` contains only `postgres` and `redis` — stateful workloads that do not roll naturally.  All other pods pick up new certs the next time they roll for any reason.  CA rotation uses a 24-hour overlap window during which both CAs are trusted, preventing hard failures mid-rotation.

**Burn-down and reinstall** (use when CA changes have left the constellation in an inconsistent state):
...
- **cert-forge** — stateless itself, but the constellation CA is persisted in the K8s Secret across restarts

All other Jobs are stateless and restart cleanly.

**Production open items (security team decisions):**
- Traefik backend CA verification — `insecureSkipVerify: true` is dev-only.  The `seti-traefik-ca` Secret is available; the access model for Traefik to use it must be defined by the security team.
- cert-forge RBAC scope — current Role includes `patch` on Deployments for rotation.  Review scope before production.
- etcd encryption at rest — K8s Secrets are base64-encoded by default.  Enable encryption at rest or use an external secrets manager.
...
|------|---------|----------|
| 4000 | gateway | Yes — sole external entry point (HTTPS) |
...
**Per-constellation cert-forge, not cluster-level.**  cert-forge is deployed per constellation, not shared across the cluster.  Different constellations may have different CA backends, cert lifetimes, or organizational CA requirements.  Centralizing cert-forge would collapse those decisions into a shared policy and mean a single cert-forge restart breaks every constellation simultaneously.

**Gateway external listener is always HTTPS.**  The gateway speaks HTTPS on its external port regardless of what sits in front of it.  Plain HTTP between Traefik and the gateway is unacceptable in a zero-trust environment — the segment is inside the cluster but it is not encrypted without TLS on the gateway side.

**Traefik backend CA verification is a production security decision.**  In dev, `insecureSkipVerify: true` in the `ServersTransport` is used because Traefik v2 requires CA Secrets in `kube-system`, and granting cert-forge write access to `kube-system` belongs to the security team.  The `seti-traefik-ca` Secret is generated and waiting.  This is a deferred decision, not an omission.
