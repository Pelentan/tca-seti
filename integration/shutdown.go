...
//
// Contract: contracts/openapi/integration.yaml x-tca-lifecycle
...
func awaitShutdown(server *http.Server) {
	...
	sig := <-quit
	log.Printf("[integration] Received %s — starting graceful shutdown", sig)
	...
	if err := server.Shutdown(ctx); err != nil {
		log.Printf("[integration] HTTP server shutdown error: %v", err)
	}

	log.Printf("[integration] Shutdown complete")
	os.Exit(0)
}
